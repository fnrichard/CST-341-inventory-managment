...
public class UserDatabase implements DataAccessInterfaceUser{
	...
	0: There were errors with the mySQL connection and update;
	1: The update was successful and new user was inserted;
	2: There was already a user with Username and Email credentials in the database;
	...
	*/
	...
	public boolean update(User t) {
		...
		for (User users : findAll()) {
			if (users.getUsername().equals(t.getUsername()) || users.getPassword().equals(t.getPassword()) || users.getEmail().equals(t.getEmail())) {
				id = users.getID();
				foundUser = users;
			}
		}
